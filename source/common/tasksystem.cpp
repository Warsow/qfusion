...
#ifndef TASK_SYSTEM_NO_CUSTOM_MUTEX
// We have to use a custom futex-based mutex implementation
// which turns out to be sufficiently good for this specific purpose.
// Other parts of the codebase still use well-tested default/generic platform facilities.
// Implementation of this mutex is derived from the following code:
...
public:
	void lock() {
		if( m_state.exchange( kLocked, std::memory_order_acquire ) != kUnlocked ) [[unlikely]] {
			while( m_state.exchange( kSleeper, std::memory_order_acquire ) != kUnlocked ) {
				m_state.wait( kSleeper, std::memory_order_relaxed );
			}
		}
	}
	void unlock() {
		if( m_state.exchange( kUnlocked, std::memory_order_release ) == kSleeper ) [[unlikely]] {
			m_state.notify_one();
		}
	}
private:
	std::atomic<uint32_t> m_state { kUnlocked };

	static constexpr uint32_t kUnlocked = 0;
	static constexpr uint32_t kLocked   = 1;
	static constexpr uint32_t kSleeper  = 2;
};
// The borrowed mutex code ends
#else
using TapeMutex = wsw::Mutex;
#endif
...
bool TaskSystem::threadExecTasks( TaskSystemImpl *__restrict impl, unsigned threadNumber ) {
	...
	try {
		...
		for(;; ) {
			...
			do {
				[[maybe_unused]] volatile wsw::ScopedLock<TapeMutex> lock( &impl->globalMutex );
