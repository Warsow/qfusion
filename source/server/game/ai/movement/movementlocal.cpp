...
auto findRampClusterExitReachNumAndTravelTime( const AiEntityPhysicsState &entityPhysicsState,
	...
	class Walker : public ReachChainWalker {
	public:
		int foundReachNum { 0 };
		int foundTravelTime { 0 };
		Walker( const AiAasRouteCache *routeCache_, int travelFlags_ ) : ReachChainWalker( routeCache_, travelFlags_ ) {}
		bool Accept( int reachNum, const aas_reachability_t &reach, int travelTime ) override {
			if( reach.traveltype != TRAVEL_WALK ) {
				return false;
			}
			if( !( AiAasWorld::instance()->getAreaSettings()[reach.areanum].areaflags & AREA_INCLINED_FLOOR ) ) {
				return false;
			}
			return true;
		}
		bool Exec() override {
			...
			if( ReachChainWalker::Exec() ) {
				if( aasWorld->getReaches()[lastReachNum].traveltype == TRAVEL_WALK ) {
					for( int i = 0; i < numStartAreas; ++i ) {
						// If some of the start areas was inclined
						// (we assume that callers take care of not calling the enclosing subroutine in other areas)
						if( aasWorld->getAreaSettings()[startAreaNums[i]].areaflags & AREA_INCLINED_FLOOR ) {
							foundReachNum   = lastReachNum;
							foundTravelTime = lastTravelTime;
							return true;
						}
					}
				}
	...
	Walker walker( bot->RouteCache(), bot->TravelFlags() );
	walker.SetAreaNums( entityPhysicsState, bot->NavTargetAasAreaNum() );
	if( walker.Exec() ) {
		assert( walker.foundReachNum > 0 );
		return std::make_pair( walker.foundReachNum, walker.foundTravelTime );
	}
