...
#else
		setsockopt( handle, IPPROTO_IPV6, IPV6_V6ONLY, (const char*)&ipv6_only, sizeof( ipv6_only ) );
#endif
	}
#endif
...
#ifndef _WIN32
	ssize_t res = ::sendto( socket->handle, data, length, 0, (struct sockaddr *)&addr, addrlen );
#else
	int64_t res = ::sendto( socket->handle, (const char *)data, (int)length, 0, (struct sockaddr *)&addr, (int)addrlen );
#endif
...
#ifndef _WIN32
	ssize_t ret = recv( socket->handle, data, length, 0 );
#else
	int64_t ret = recv( socket->handle, (char *)data, (int)length, 0 );
#endif
...
#ifndef _MSC_VER
	ssize_t ret = recv( socket->handle, (char*)buffer, sizeof( buffer ), MSG_PEEK );
#else
	int64_t ret = recv( socket->handle, (char*)buffer, sizeof( buffer ), MSG_PEEK );
#endif
...
#ifndef _WIN32
	ssize_t ret = ::send( socket->handle, data, length, MSG_NOSIGNAL );
#else
	int64_t ret = ::send( socket->handle, (const char *)data, (int)length, MSG_NOSIGNAL );
#endif
