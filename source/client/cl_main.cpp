...
void CL_GameModule_Init( void ) {
	...
	CG_Init( RF_GetRenderSystem(), soundSystem, wsw::ui::UISystem::instance(), cls.servername, cl.playernum,
			 VID_GetWindowWidth(), VID_GetWindowHeight(), VID_GetPixelRatio(),
...
void SCR_UpdateScreen( void ) {
	...
	const unsigned videoWidth  = VID_GetWindowWidth();
	const unsigned videoHeight = VID_GetWindowHeight();

	const bool shouldDrawConsole = canDrawConsole && scr_con_current > 0.0f;
	if( canDrawDebug && !shouldDrawConsole ) {
		Con_DrawNotify( renderSystem, videoWidth, videoHeight );
		CL_ProfilerHud_Draw( renderSystem, videoWidth, videoHeight );
	}

	if( shouldDrawConsole ) {
		Con_DrawConsole( renderSystem, videoWidth, videoHeight * scr_con_current );
	}
...
void CL_Precache_f( const CmdArgs &cmdArgs ) {
	...
	if( cls.demoPlayer.playing ) {
		if( !cls.demoPlayer.play_jump ) {
			...
			CL_GameModule_Reset();
			SoundSystem::instance()->stopSounds( SoundSystem::RetainLocal );
		}
...
*/
void CL_S_Restart( const CmdArgs &cmdArgs ) {
	const bool verbose = ( Cmd_Argc() >= 2 ? true : false );
...
void CL_AdjustServerTime( unsigned int gameMsec ) {
	...
	if( oldServerTime > cl.serverTime ) {
		// Reset timestamp-dependent effects
		if( cge ) {
			CG_ClearEffects();
		}
	}
