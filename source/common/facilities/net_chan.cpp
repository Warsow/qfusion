...
bool Netchan_TransmitNextFragment( netchan_t *chan ) {
	...
	if( showpackets->integer ) {
		Com_Printf( "%s send %4" PRIu64 ": s=%i fragment=%" PRIu64 ",%i\n",
			NET_SocketToString( chan->socket ), (uint64_t)send.cursize,
			chan->outgoingSequence, (uint64_t)chan->unsentFragmentStart, fragmentLength );
	}
...
bool Netchan_Transmit( netchan_t *chan, msg_t *msg ) {
	...
	if( msg->cursize > MAX_MSGLEN ) {
		Com_Error( ERR_DROP, "Netchan_Transmit: Excessive length = %" PRIu64, (size_t)msg->cursize );
		return false;
	}
	...
	if( showpackets->integer ) {
		Com_Printf( "%s send %4" PRIu64 " : s=%i ack=%i\n", NET_SocketToString( chan->socket ), (uint64_t)send.cursize,
...
bool Netchan_Process( netchan_t *chan, msg_t *msg ) {
	...
	if( showpackets->integer ) {
		if( fragmented ) {
			Com_Printf( "%s recv %4" PRIu64 " : s=%i fragment=%i,%i\n", NET_SocketToString( chan->socket ),
				(uint64_t)msg->cursize, sequence, fragmentStart, fragmentLength );
		} else {
			Com_Printf( "%s recv %4" PRIu64 " : s=%i\n", NET_SocketToString( chan->socket ),
				(uint64_t)msg->cursize, sequence );
		}
	}
	...
	if( fragmented ) {
		...
		if( fragmentStart != (int) chan->fragmentLength ) {
			if( showdrop->integer || showpackets->integer ) {
				Com_Printf( "%s:Dropped a message fragment for sequence %d\n",
					NET_AddressToString( &chan->remoteAddress ), sequence );
			}
		...
		if( chan->fragmentLength > msg->maxsize ) {
			Com_Printf( "%s:fragmentLength %" PRIu64 " > msg->maxsize\n",
				NET_AddressToString( &chan->remoteAddress ), (uint64_t)chan->fragmentLength );
			return false;
		}
