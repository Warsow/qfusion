...
void GClip_BackUpCollisionFrame( void ) {
	...
	for( i = 0; i < game.numentities; i++ ) {
		...
		if( !svedict->r.inuse || svedict->r.solid == SOLID_NOT
			|| ( svedict->r.solid == SOLID_TRIGGER && !( i >= 1 && i <= ggs->maxclients ) ) ) {
			continue;
		}
...
static c4clipedict_t *GClip_GetClipEdictForDeltaTime( int entNum, int deltaTime ) {
	...
	if( !ent->r.inuse || ent->r.solid == SOLID_NOT
		|| ( ent->r.solid == SOLID_TRIGGER && !( entNum >= 1 && entNum <= ggs->maxclients ) ) ) {
		clipent->r = ent->r;
		clipent->s = ent->s;
		return clipent;
	}
...
static void GClip_Init_AreaGrid( areagrid_t *areagrid, const vec3_t world_mins, const vec3_t world_maxs ) {
	...
	areagrid->size[0] = wsw::max( world_maxs[0] - world_mins[0], AREA_GRID * AREA_GRIDMINSIZE );
	areagrid->size[1] = wsw::max( world_maxs[1] - world_mins[1], AREA_GRID * AREA_GRIDMINSIZE );
	areagrid->size[2] = wsw::max( world_maxs[2] - world_mins[2], AREA_GRID * AREA_GRIDMINSIZE );
...
static int GClip_EntitiesInBox_AreaGrid( areagrid_t *areagrid, const vec3_t mins, const vec3_t maxs,
	...
	igridmins[0] = wsw::max( 0, igridmins[0] );
	igridmins[1] = wsw::max( 0, igridmins[1] );

	//igridmins[2] = max( 0, igridmins[2] );
	igridmaxs[0] = wsw::min( AREA_GRID, igridmaxs[0] );
	igridmaxs[1] = wsw::min( AREA_GRID, igridmaxs[1] );
...
void G_PMoveTouchTriggers( pmove_t *pm, const vec3_t previous_origin ) {
	...
	if( pm->playerState->POVnum <= 0 || (int)pm->playerState->POVnum > ggs->maxclients ) {
		return;
	}
...
void G_SplashFrac( int entNum, const vec3_t hitpoint, float maxradius, vec3_t pushdir,
	...
	clipEnt = GClip_GetClipEdictForDeltaTime( entNum, 0 );
