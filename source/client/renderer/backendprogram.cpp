...
#define DRAWFLAT( state ) \
	( ( ( state )->m_materialState.currentModelType == mod_brush ) && \
	( ( state )->m_globalState.renderFlags & ( RF_DRAWFLAT | RF_DRAWBRIGHT ) ) && \
	!( ( state )->m_materialState.currentShader->flags & SHADER_NODRAWFLAT ) )
...
SimulatedBackendState::SimulatedBackendState( BackendActionTape *actionTape_, int width, int height )
	: m_glState( actionTape_, width, height ), m_actionTape( actionTape_ ) {
	...
	std::memset( &m_uniformState, 0, sizeof( m_uniformState ) );
...
void SimulatedBackendState::setTime( int64_t time ) {
	m_globalState.time = time;
	// TODO?????????????????????????????????????????????????? Pass as an argument
	m_globalState.nullEnt.shaderTime = Sys_Milliseconds();
}
...
void SimulatedBackendState::drawMeshVerts( const DrawMeshVertSpan *vertSpan, int primitive ) {
	if( m_materialState.currentEntity != &m_globalState.nullEnt ) [[likely]] {
		m_glState.applyScissor();
...
void SimulatedBackendState::setupProgram( int type, const shader_s *materialToGetDeforms, uint64_t features ) {
...
void SimulatedBackendState::updateCommonUniforms( const shaderpass_t *pass, mat4_t texMatrix ) {
	...
	const auto glStateFlags = m_glState.getState();
	vec2_t blendMix { 0, 0 };
	if( RB_IsAlphaBlending( glStateFlags & GLSTATE_SRCBLEND_MASK, glStateFlags & GLSTATE_DSTBLEND_MASK ) ) {
		blendMix[1] = 1;
...
void SimulatedBackendState::renderMeshUsingQ3AProgram( const FrontendToBackendShared *fsh,
	...
	setupProgram( GLSL_PROGRAM_TYPE_Q3A_SHADER, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingMaterialProgram( const FrontendToBackendShared *fsh,
	...
	setupProgram( GLSL_PROGRAM_TYPE_MATERIAL, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingDistortionProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_DISTORTION, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingOutlineProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_OUTLINE, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingCelshadeProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_CELSHADE, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingFogProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_FOG, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingFxaaProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_FXAA, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingYuvProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_YUV, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingColorCorrectionProgram( const FrontendToBackendShared *,
	...
	setupProgram( GLSL_PROGRAM_TYPE_COLOR_CORRECTION, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::renderMeshUsingKawaseProgram( const FrontendToBackendShared *fsh,
	...
	setupProgram( GLSL_PROGRAM_TYPE_KAWASE_BLUR, m_materialState.currentShader, programFeatures );
...
void SimulatedBackendState::drawShadedMesh( const FrontendToBackendShared *fsh, const DrawMeshVertSpan *vertSpan, int primitive ) {
	if( !tryExecutingSinglePassReusingBoundState( vertSpan, primitive ) ) {
		...
		if( ENTITY_OUTLINE( m_materialState.currentEntity ) ) {
