...
static unsigned MSG_CompareArrays( const void *from, const void *to, const msg_field_t *field, uint8_t *elemMask, size_t maskSize, bool quick ) {
	...
	const uint8_t *bfrom = (uint8_t *)from, *bto = (uint8_t *)to;
...
static unsigned MSG_CompareStructs( const void *from, const void *to, const msg_field_t *fields, size_t numFields, uint8_t *fieldMask, size_t maskSize ) {
	...
	for( i = 0; i < numFields; i++ ) {
		...
		if( f->count > 1 ) {
			change = MSG_CompareArrays( (const uint8_t *)from, (const uint8_t *)to, f, NULL, 0, true ) != 0;
		} else {
			change = MSG_CompareField( (const uint8_t *)from, (const uint8_t *)to, f );
		}
		...
		if( byteMask & 1 ) {
			...
			while( fm ) {
				...
				if( fm & 1 ) {
					...
					if( f->count > 1 ) {
						MSG_WriteDeltaArray( msg, (const uint8_t *)from, (uint8_t *)to, f );
					} else {
						MSG_WriteField( msg, (uint8_t *)to, f );
					}
				}
				fn++;
				fm >>= 1;
		...
		if( byteMask & 1 ) {
			...
			while( fm ) {
				...
				if( fm & 1 ) {
					const msg_field_t *f = &fields[fn];
					if( f->count > 1 ) {
						MSG_ReadDeltaArray( msg, (const uint8_t *)from, (uint8_t *)to, f );
					} else {
						MSG_ReadField( msg, (uint8_t *)to, f );
					}
				}
...
static const msg_field_t player_state_msg_fields[] = {
	...
	{ PSOFS( pmove.unused ), 32, 1, WIRE_UBASE128 },
...
static const msg_field_t raw_scoreboard_msg_fields[] = {
	...
	{ offsetof( ReplicatedScoreboardData, pingSlot ), 8, 1, WIRE_FIXED_INT8 },
};
