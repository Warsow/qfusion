...
void CG_RegisterFonts() {
	...
	cgs.font2DTiny = SCR_RegisterFont( DEFAULT_SYSTEM_FONT_FAMILY, QFONT_STYLE_NONE, (unsigned)( 12 * scale ) );
	if( !cgs.font2DTiny ) {
		CG_Error( "Couldn't load default font \"%s\"", DEFAULT_SYSTEM_FONT_FAMILY );
	}

	cgs.font2DMedium = SCR_RegisterFont( DEFAULT_SYSTEM_FONT_FAMILY, QFONT_STYLE_NONE, (unsigned)ceilf( 14 * scale ) );
	if( !cgs.font2DMedium ) {
		CG_Error( "Couldn't load default font \"%s\"", DEFAULT_SYSTEM_FONT_FAMILY );
	}

	cgs.font2DBig = SCR_RegisterFont( DEFAULT_SYSTEM_FONT_FAMILY, QFONT_STYLE_NONE, (unsigned)( 16 * scale ) );
	if( !cgs.font2DBig ) {
		CG_Error( "Couldn't load default font \"%s\"", DEFAULT_SYSTEM_FONT_FAMILY );
	}
}
