...
static void SV_Physics_Toss( edict_t *ent ) {
	...
	if( !wasinwater && isinwater ) {
		G_PositionedSound( old_origin, CHAN_AUTO, SV_SoundIndex( S_HIT_WATER ), ATTN_IDLE );
	} else if( wasinwater && !isinwater ) {
		G_PositionedSound( ent->s.origin, CHAN_AUTO, SV_SoundIndex( S_HIT_WATER ), ATTN_IDLE );
	}
...
void SV_Physics_LinearProjectile( edict_t *ent, int lookAheadTime ) {
	...
	if( !old_waterLevel && ent->waterlevel ) {
		G_PositionedSound( start, CHAN_AUTO, SV_SoundIndex( S_HIT_WATER ), ATTN_IDLE );
